...
export class RsvpController {
  ...
  static async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (phone_number === undefined || phone_number === null) {
        ...
        if (digits.length < 8) {
          errors.push(
            "O número de telefone fornecido em 'phone_number' é muito curto (mínimo 8 dígitos numéricos).",
          );
        } else if (digits.length > 15) {
          errors.push(
            "O número de telefone fornecido em 'phone_number' é muito longo (máximo 15 dígitos numéricos).",
          );
        }
      }
      ...
      if (will_go === undefined || will_go === null) {
        ...
        errors.push(
          "O campo 'will_go' deve ser um valor booleano (true ou false).",
        );
      }
      ...
      if (error.code === "P2002") {
        ...
        const friendlyField = fieldName.includes("email")
          ? "e-mail"
          : "telefone";
        res.status(400).json({
          status: "error",
          message: `O ${friendlyField} informado já foi confirmado.`,
        });
        return;
      }
  ...
  static async lookup(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formattedPhone = sanitizeAndFormatPhone(phone_number);
      const rsvp = await RsvpService.findRsvpByEmailAndPhone(
        email.trim(),
        formattedPhone,
      );
      ...
      if (!rsvp) {
        res.status(404).json({
          status: "error",
          message: "Ninguém se cadastrou com este e-mail e telefone.",
        });
        return;
      }
